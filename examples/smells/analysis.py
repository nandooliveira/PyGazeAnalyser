...
for ppname in PPS:
	...
	print("plotting gaze data")
        ...
        for point in data:
            ...
            fixations.append([time.time(), time.time(), 1, int(point[0]), int(point[1])])

            if len(previous_point) != 0:
                saccades.append([time.time(), time.time(), 1, int(previous_point[0]), int(previous_point[1]), int(point[0]), int(point[1])])
